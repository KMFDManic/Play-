...
travis_before_install() 
{
    if [ "$TARGET_OS" = "Linux" ]; then
        ...
        echo "ndk.dir=$ANDROID_NDK">./build_android/local.properties
...
travis_script()
{
    if [ "$TARGET_OS" = "Android" ]; then
        ...
        if [ "$TARGET_OS" = "Linux" ]; then
            ...
            cmake .. -G"$BUILD_TYPE" -DBUILD_LIBRETRO_CORE=yes -DCMAKE_PREFIX_PATH=/opt/qt512/ -DCMAKE_INSTALL_PREFIX=./appdir/usr;
            cmake --build .
            ...
            cmake .. -G"$BUILD_TYPE" -DBUILD_LIBRETRO_CORE=yes -DCMAKE_TOOLCHAIN_FILE=../deps/Dependencies/cmake-ios/ios.cmake -DTARGET_IOS=ON -DBUILD_PSFPLAYER=ON
            cmake --build . --config Release
